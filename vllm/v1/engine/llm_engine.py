...
class LLMEngine:
    ...
    def __init__(
        ...
        if self.tokenizer and vllm_config.scheduler_config.enable_gate_optimization:
            # TODO(wk):check configs validity
            try:
                probe_suffix = " Please determine whether the prefix kv cache generated by the above prompt will have a high hit rate in subsequent inferences. Output only yes/no.\n\n"
                probe_suffix_ids = self.tokenizer.encode(probe_suffix)
                yes_ids = self.tokenizer.encode("yes")
                no_ids = self.tokenizer.encode("no")
                vllm_config.scheduler_config.gate_probe_ids = {
                    "probe_suffix_ids": probe_suffix_ids,
                    "yes_id": yes_ids[0],
                    "no_id": no_ids[0]
                }
            except Exception as e:
                logger.warning(f"Failed to initialize Gate Mechanism IDs: {e}")
